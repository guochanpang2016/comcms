...
class Menu extends Validate
{
    ...
    protected $rule = [
        ...
        ['module', 'require|alphaDash|max:50', '模型不能为空|模型格式不正确|模型不能超过50个字符'],
        ['controller', 'require|alphaDash|max:50', '控制器不能为空|控制器格式不正确|控制器不能超过50个字符'],
        ['action', 'require|alphaDash|max:50', '方法不能为空|方法格式不正确|方法不能超过50个字符'],
        ['data', 'max:50', '额外参数不能超过50个字符'],
        ['type', 'number|in:0,1', '菜单类型必须是数字|菜单类型，1：权限认证+菜单；0：只作为菜单'],
        ['name', 'require|max:50', '名称不能为空|名称不能超过50个字符'],
